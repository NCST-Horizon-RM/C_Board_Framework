...
uint8_t Chassis_Control_Init(void)
{
    ...
    float PID_V_Param[3] = {10.0f, 0.0f, 0.0f};
    PID_Init(&chassis_ctrl.PID_Vx, 10.0f, 5.0f, PID_V_Param,0, 0, 0, 0, 0, Integral_Limit | ErrorHandle);
    PID_Init(&chassis_ctrl.PID_Vy, 10.0f, 5.0f, PID_V_Param,0, 0, 0, 0, 0, Integral_Limit | ErrorHandle);

    float PID_Vw_Param[3] = {15.0f, 0.0f, 0.0f};
    PID_Init(&chassis_ctrl.PID_Vw, 15.0f, 8.0f, PID_Vw_Param,0, 0, 0, 0, 0, Integral_Limit | ErrorHandle);
                          ...
                          const IMU_Data_t *c_imu)
{
    ...
    if (cmd.mode == CHASSIS_CMD_SAFE)
    {
        ...
        DJI_Motor_Send(&hcan2, 0x1FE,0,0,0,0);
    }
    else
    {
