...
inline float ARM_Atan2_Fast(float y, float x) {
    ...
    // 如果计算失败（例如传入参数无效），status 为 ARM_MATH_ARGUMENT_ERROR
    // 但通常 x 和 y 都是有效浮点数，不会失败，可忽略 status
    (void)status;   // 防止未使用警告
    return result;  // 返回弧度
}
 ...
 */
inline float ARM_Sin_Fast(float angle_deg) {
    float rad = angle_deg * 0.01745329252f;   // 度转弧度
 ...
 */
inline float ARM_Cos_Fast(float angle_deg) {
